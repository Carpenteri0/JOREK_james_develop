!> Module to read file of thermal background temperatures.
!> Used when including a population of energetic particles
!> which carry their own pressure. Need to then seperate
!> the thermal temp from EP temp for force balance
!>
!> Originally written by F. Antlitz
!> Ported to kinetic_main by J. Carpenter
!> Aug, 2026
!> zhpw96@durham.ac.uk


subroutine temperature_thermal(xpoint2,xcase2,Z,Z_xpoint,psi,psi_axis,psi_bnd,temperature_profile, &
...
if ( .not. num_T_thermal ) then

   write(*,*) "Initialization with equilibirum taking particle pressure into account needs a numerical T_profile!"
   stop
  ...
  left  = 1
  right = num_T_thermal_len
  do
    if ( right == left + 1 ) exit
    mid = (left + right) / 2
    if ( num_T_thermal_x(mid) >= psi_n ) then
      right = mid
    else
      left = mid
    end if
  end do
  aux1 = (psi_n - num_T_thermal_x(left)) / (num_T_thermal_x(right) - num_T_thermal_x(left))
  aux2 = (1. - aux1)
  prof1        = num_T_thermal_y0(left)   * aux2 + num_T_thermal_y0(right) * aux1
  dprof1_dpsi  = ( num_T_thermal_y1(left) * aux2 + num_T_thermal_y1(right) * aux1 ) / delta_psi
  dprof1_dpsi2 = ( num_T_thermal_y2(left) * aux2 + num_T_thermal_y2(right) * aux1 ) / delta_psi**2
  dprof1_dpsi3 = ( num_T_thermal_y3(left) * aux2 + num_T_thermal_y3(right) * aux1 ) / delta_psi**3
...
return
end subroutine temperature_thermal
